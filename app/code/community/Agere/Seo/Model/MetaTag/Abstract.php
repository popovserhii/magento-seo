...
abstract class Agere_Seo_Model_MetaTag_Abstract implements Agere_Seo_Model_MetaTag_MetaInterface {

	/**
	 * Rule collection
	 *
	 * @var Agere_Seo_Model_Resource_Rule_Collection
	 */
	protected $rules;
	...
	public function __construct(Agere_Seo_Model_Resource_Rule_Collection $rules) {
		$this->rules = $rules;
	}
	 ...
	 * Add rule item
	 *
	 * @param Agere_Seo_Model_Rule $rule
	 * @return $this
	 */
	public function addRule(Agere_Seo_Model_Rule $rule) {
	 ...
	 *
	 * @return Agere_Seo_Model_Resource_Rule_Collection
	 */
	...
	protected function prepareTranslate($string) {
		return Mage::helper('agere_seo')->__($string);
	}

	protected function prepareTranslit($string) {
		static $trans = false;
		if (!$trans) { // little cache
			$trans = Mage::getConfig()->getNode("global/dictionaries/agere_seo/transliterations");
		}
	...
	protected function getFittingRule() {
		if (!$this->fittingRule) {
			...
			foreach ($rules as $key => $rule) {
				...
				// and sort Agere_Seo_Model_MetaTag_Category::getFittingFilterAttributes() ~207 line
				$best[$key] = 0;
