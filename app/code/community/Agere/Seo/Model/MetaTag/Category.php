 ...
 */
class Agere_Seo_Model_MetaTag_Category extends Agere_Seo_Model_MetaTag_Abstract {
	...
	protected function postRun() {
		...
		if (Mage::helper('agere_seo')->hasFilters()) {
	...
	protected function noindex($layerModel) {
		...
		if (($head = Mage::getSingleton('core/layout')->getBlock('head'))) {
			...
			foreach (explode(',', Mage::getStoreConfig('agere_section/settings/no_index')) as $noIndexProcessorName) {
				...
				$noIndexProcessor = Mage::getModel((string)Mage::getConfig()->getNode('agere_seo_filterlinks/noindex')->$noIndexProcessorName->model);
			...
			foreach (explode(',', Mage::getStoreConfig('agere_section/settings/follow')) as $followProcessorName) {
				...
				$followProcessor = Mage::getModel((string)Mage::getConfig()->getNode('agere_seo_filterlinks/noindex')->$followProcessorName->model);
	...
	protected function index($layerModel) {
		...
		if (($head = Mage::getSingleton('core/layout')->getBlock('head'))) {
			...
			foreach (explode(',', Mage::getStoreConfig('agere_section/settings/index')) as $indexProcessorName) {
				...
				$indexProcessorConfig = Mage::getConfig()->getNode('agere_seo_filterlinks/index')->$indexProcessorName;
	...
	protected function prepareLinkRel() {
		if (Mage::getStoreConfig('agere_section/settings/rel_prev_next')) {
			...
			if ($head) {
				...
				if ($linkPrev) {
					$head->addLinkRel('prev', Mage::helper('agere_seo')->urlPageNormalize($prevUrl));
				}
				if ($linkNext) {
	...
	/*protected function getGeneralEndingOfMetaTags() {
		...
		if (is_null($end)) {
			...
			if (Mage::getStoreConfig('agere_section/settings/pager')) {
				...
				if ($this->isPageInUrl()) {
					if ($tool->getLastPageNum() > 1) {
						$end = Mage::helper('agere_seo')->__('page') . ' ' . $tool->getCurrentPage();
					}
				}
			}
		}
